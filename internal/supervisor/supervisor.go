...
func (manager *Manager) StartService(ctx context.Context, processID string) (domain.Run, error) {
	...
	manager.mu.Unlock()
	return manager.launch(ctx, process)
}
...
func (manager *Manager) command(process domain.Process, runID string) (*exec.Cmd, error) {
	if len(process.Command.Argv) > 0 {
		arguments := make([]string, len(process.Command.Argv))
...
func (manager *Manager) environment(process domain.Process, runID string) []string {
	values := append([]string(nil), os.Environ()...)
	values = append(values,
		"PROC_MAN_PROCESS_ID="+process.ID,
		"PROC_MAN_RUN_ID="+runID,
	)
